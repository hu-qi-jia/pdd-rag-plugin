...
> 版本 2.6.18 · 2026-09-16 · 对齐 Figma 编辑器工具界面(与 pddddd 控制台同一设计语言;v2.0 的 Figma 营销官网风整体替换)
...
- `.pddcs-popup`:候选弹窗(v2.6.18 重设计:**三段式浮层壳** —— 头/页脚常驻,滚动只发生在
  ...
  入场 160ms 淡入上移)。8px 圆角 + 柔和双层浮层阴影;候选行为**内缩圆角软行**
  (v2.6.16 去 1px 分隔线靠留白分组,v2.6.18 升级为 margin 内缩 + 8px 圆角,
  悬浮 = 选中同一软中性灰填充;**操作钮悬浮/选中才显**),类别徽标**降级 6px 小圆点 + 灰字**
  (点色同源 semantic 金/绿,历史 = 中性灰),页脚键位**键帽化** `.pddcs-kbd`。
