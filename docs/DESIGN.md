...
> 版本 2.6.12 · 2026-09-16 · 对齐 Figma 编辑器工具界面(与 pddddd 控制台同一设计语言;v2.0 的 Figma 营销官网风整体替换)
...
- **键盘导航**(v2.6.12,仅快捷键唤起的面板):打开即选中第一条(高亮 = accent 左描边
  `inset 2px` + `btnHoverBg` 底);`↓`/`↑` 移动选中项,端点**夹取不回绕**
  (`pddUiLogic#moveSelection`),并 `scrollIntoView(nearest)` 滚进可视区;`Enter` 填充
  **选中项**(原固定第一条),脚注提示「↑↓ 选择,Enter 填充;发送请手动点击」。
  鼠标悬浮把选中态一并带过去 —— 两套高亮共用一态,不出现 hover 底色与选中描边打架;
  点击「AI回复」打开的面板不进入键盘模式(不拦 ↑↓/Enter,保持页面原有键位)。
