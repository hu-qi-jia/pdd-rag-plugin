...
> 版本 2.5 · 2026-09-15 · 对齐 Figma 编辑器工具界面(与 pddddd 控制台同一设计语言;v2.0 的 Figma 营销官网风整体替换)
...
| `btnPrimaryBg` | `#161616`(白字) | `#0D99FF`(白字) | 主按钮(浅色黑白;深色 Figma 蓝) |
...
## 七-B、文件夹页分区容器化(v2.4 重设计)

用户反馈"文件夹的表示混乱":原设计里文件夹头、子文件夹、标准回答全部平铺在同一条分隔线流里,
子夹还夹在父夹内容中间。重设计(工具风 + 资源管理器直觉):

- **根文件夹 = 一张分区容器**:1px 边框 + 8px 圆角 + `overflow:hidden`,把"文件夹"与"里面的答复"框在一起;
  头部为标题栏(常驻 `bgSecondary` 底,展开时底边一条 `borderLight`,悬浮 `borderLight` 加深)。
- **文件夹优先于内容**:子文件夹排在父夹标准回答**之前**(原先排在其后),缩进 12px + 1px 引导线;
  子夹头透明底、字号降为 secondary、名称用 `textMuted`,与根夹拉开层级。
- **内容居左**(v2.5 用户要求):标准回答行 `padding-left = 12 + 12×深度`,行悬浮 `bgSecondary`。
- **空文件夹**:显示灰字"暂无标准回答"占位,不再只靠「0」计数表达。
- **未分类**置底,容器用**虚线边框** + 图标/名称 muted,弱化存在感。
- **工具栏**:「+ 新建根文件夹」左对齐(与知识库按钮行同款),内联新建时原位变表单。
...
- **来源保障**:面板必须展示 标准答案 > 历史 > 知识库 三类(存在且文本未展示过时),
  折叠/截断吞掉的类别由检索装配补一条最优代表(`retrieval.ts` 尾部类别保障)。
...
- `.pddcs-rail-btn`:导航图标按钮 —— **默认态只保留图标,无背景块**(v2.5 用户要求),
  激活态用图标颜色(`text`)与描边粗细(2.4 vs 1.8)表达;悬浮底色走 `--pddcs-rail-hover`;
  导航栏顶部**不放品牌标**。
- `.pddcs-row(-ops)`:行悬浮操作显现规则(见 §五)。
- **令牌 → CSS 变量桥**:静态 CSS(RESET_CSS)读不到 React 令牌,凡随主题变化的静态规则一律走
  CSS 变量:`--pddcs-scroll-thumb`(滚动条滑块)、`--pddcs-rail-hover`(导航钮悬浮底)。
  注意**内联样式优先级高于类选择器**:未激活的导航钮不能内联写 `background: transparent`,
  否则 `:hover` 永远不生效。
