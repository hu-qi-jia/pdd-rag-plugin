...
> 版本 2.6.17 · 2026-09-16 · 对齐 Figma 编辑器工具界面(与 pddddd 控制台同一设计语言;v2.0 的 Figma 营销官网风整体替换)
...
- `.pddcs-popup`:候选弹窗(8px 圆角 + 柔和双层浮层阴影;v2.6.16 ChatGPT 化:
  候选行**去 1px 分隔线**靠留白 + 悬浮底色分组,类别徽标**降级 6px 小圆点 + 灰字**
...
- **键盘导航**(v2.6.12 引入,v2.6.13 改键,v2.6.14 改灰,v2.6.15 循环化,仅快捷键唤起的面板):打开即选中第一条
  ...
  **回绕到首条**,v2.6.15 用户指定;Shift+Tab 反向已删,`panelNavKeys` 一并移除,
  `pddUiLogic#moveSelection`),选中行滚进可视区(v2.6.17:sticky 头会盖住
  `scrollIntoView(nearest)` 对齐位,改**头部实高感知**手动滚动,选中首条直接 scrollTop=0);
  `Enter` 填充
