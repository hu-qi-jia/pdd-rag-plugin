...
for (const theme of ['light', 'dark']) {
  ...
  for (const [id, label] of TABS) {
    ...
    await sleep(700)
    await pop.screenshot({ path: path.join(ROOT, 'logs', `ui-0915-${TAG}-${id}-${theme}.png`), clip: { x: 0, y: 0, width: 460, height: 620 } })
  }
...
await pop.screenshot({
  path: path.join(ROOT, 'logs', `ui-0915-${TAG}-memory-paged-light.png`),
...
await sleep(400)
await pop.screenshot({
  path: path.join(ROOT, 'logs', `ui-0915-${TAG}-settings-storage-light.png`),
  clip: { x: 0, y: 0, width: 460, height: 620 },
})
